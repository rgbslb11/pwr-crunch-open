// POWER CRUNCH v2.0.4 base runtime: direct Week-4 deployment ratings through Week 3.
(function(){
const PRODUCT_VERSION='PC-MOBILE-v2.0.4';
const DATA_VERSION='2026-week4-canonical-ratings-v204-fcs60';
const FCS=new Set(window.PC_FCS_CODES||[]);
const CFG={
  marginPerTeamRating:0.75,
  homeField:2.5,neutralField:0.0,
...
window.PC_MODEL_NAME='v2.0.4 canonical TEAM/OFF/DEF ratings + centered unit-total adjustment + environmental state';
...
ratingCard=function(t){const nOverall=human('overall',t),nOff=human('offense',t),nDef=human('defense',t),src=FCS.has(t.short)?'Synthetic FCS floor':'v2.0.4 canonical source';return`<div class="team-card"><h3>${t.team}</h3><div class="conf">${t.conference} · National #${R.overall[t.short]}</div><div class="rating-line"><span>Overall</span><b>${nOverall}</b><em>${label(nOverall)}</em></div><div class="rating-line"><span>Offense</span><b>${nOff}</b><em>${label(nOff)}</em></div><div class="rating-line"><span>Defense</span><b>${nDef}</b><em>${label(nDef)}</em></div><details><summary>Deployment details</summary><div class="raw">${src}<br>TEAM ${nOverall} · OFF ${nOff} · DEF ${nDef}<br>Margin bridge ${CFG.marginPerTeamRating.toFixed(2)} pts/rating · HFA ${CFG.homeField.toFixed(1)} pts</div></details></div>`;};
