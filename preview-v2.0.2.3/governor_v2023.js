...
// Preseason Board I-K powers for all teams flagged as having a Week-1 movement in the post-W1 adapter.
const PRE={
  USC:0.7168186668100102,
  UVA:0.6781795285792172,
  VAN:0.6617590140832512,
  UNC:0.4502719249889766,
  TCU:0.5941094204328974,
  FSU:0.4718778582376864,
  NCSU:0.5176188159181938,
  MEM:0.4361337635676524,
  ARK:0.4361530366318503,
  STAN:0.4171287165857034,
  UK:0.4701186364549718,
  WVU:0.32117282529108,
  COR:0.3297598488645677,
  HAW:0.3364228697193304,
  NMSU:0.3270540540540541,
  SJSU:0.2894404497472096,
  YALE:0.2897780105451401,
...
for(const [code,v] of Object.entries(P)){
  ...
  if(v.w1===1){
    ...
    const raw=v.p;
    const delta=raw-pre;
    const effective=pre+WEEK1_WEIGHT*delta;
    v.pPreseason=pre;
...
// Recompute only the overall national rank from governed effective Power.
try{
  const ranked=Object.entries(P).filter(([,v])=>v.r!==null&&Number.isFinite(v.p)).sort((a,b)=>b[1].p-a[1].p);
  ranked.forEach(([code,v],i)=>{
    v.rEffective=i+1;
    if(typeof R!=='undefined'&&R.overall) R.overall[code]=i+1;
  });
...
audit=function(){
  const x=priorAudit();
  const a=P[S.a.short],h=P[S.h.short];
  x.engineVersion=PRODUCT_VERSION;
  x.dataVersion=DATA_VERSION;
  x.week1AdmissionWeight=WEEK1_WEIGHT;
  x.defaultLaterWeekWeight=DEFAULT_LATER_WEEK_WEIGHT;
  ...
  x.awayEffectivePower=a?.p;
  x.homeEffectivePower=h?.p;
  x.awayRawPostW1Power=a?.pPostW1Raw;
  x.homeRawPostW1Power=h?.pPostW1Raw;
  x.awayPreseasonPower=a?.pPreseason??a?.pPostW1Raw;
  x.homePreseasonPower=h?.pPreseason??h?.pPostW1Raw;
  x.awayWeek1DeltaAdmitted=a?.pWeek1DeltaAdmitted??0;
  x.homeWeek1DeltaAdmitted=h?.pWeek1DeltaAdmitted??0;
  return x;
};
...
ratingCard=function(t){
  ...
  return `<div class="team-card"><h3>${t.team}</h3><div class="conf">${t.conference} · Governed National #${R.overall[t.short]}</div><div class="rating-line"><span>Overall</span><b>${nOverall}</b><em>${label(nOverall)}</em></div><div class="rating-line"><span>Offense</span><b>${nOff}</b><em>${label(nOff)}</em></div><div class="rating-line"><span>Defense</span><b>${nDef}</b><em>${label(nDef)}</em></div><details><summary>Raw model ratings</summary><div class="raw">Raw post-W1 Elo ${t.overall.toFixed(2)} · PSCORE Offense ${t.offense.toFixed(2)} · PSCORE Defense ${t.defense.toFixed(2)}<br>Preseason Power ${v.pPreseason.toFixed(4)} · Raw post-W1 Power ${v.pPostW1Raw.toFixed(4)} · Effective governed Power ${v.p.toFixed(4)}<br>Week-1 raw delta ${v.pWeek1DeltaRaw>=0?'+':''}${v.pWeek1DeltaRaw.toFixed(4)} · admitted 1/9 = ${v.pWeek1DeltaAdmitted>=0?'+':''}${v.pWeek1DeltaAdmitted.toFixed(4)}</div></details></div>`;
};
