...
} from './engine_r1.mjs?v=quarterfix1';

const STORE='powerCrunchAudit_PC2042_R1';
const $=id=>document.getElementById(id);
let rows=[],teams=[],state=null,lastForecast=null,playing=false,pauseRequested=false,runSerial=0;
...
function newGame(){
  const finished=state?.final;
  runSerial++;playing=false;pauseRequested=true;state=null;lastForecast=null;
  locked(false);$('gameCard').classList.add('hidden');
  if(finished)newSeed();
}

async function stepOne(){
  if(!state||state.final||playing)return;
...
async function playQuarter(){
  if(!state||state.final||playing)return;
  const game=state,run=++runSerial;
  playing=true;pauseRequested=false;
  const inRegulation=game.regSecondsRemaining>0;
  const stopAt=inRegulation?game.regSecondsRemaining-quarterClock(game.regSecondsRemaining).seconds:null;
  const otPeriod=game.ot?.period||1;
  render();
  try{
    for(let guard=0;guard<100&&!game.final&&!pauseRequested;guard++){
      if(state!==game||run!==runSerial)return;
      advancePossession(game,{stopAtQuarterBoundary:true});
      render(false);
      if(inRegulation?game.regSecondsRemaining<=stopAt:game.ot?.period!==otPeriod)break;
      await tick(80);
    }
    if(state!==game||run!==runSerial)return;
    lastForecast=forecast(game,100);render();if(game.final)saveFinal();
  }catch(e){if(state===game)showError(e);}
  finally{
    if(state===game&&run===runSerial){playing=false;render();}
  }
}

function requestPause(){pauseRequested=true;}

function render(withForecast=true){if(!state)return;renderPregame();renderScore();renderStatus();renderTransparency();renderLedger();if(withForecast&&lastForecast){}$('playNext').disabled=state.final||playing;$('playQuarter').disabled=state.final||playing;$('pause').disabled=!playing;$('playQuarter').textContent=state.final?'Final':state.regSecondsRemaining>0?`Play Quarter ${regulationQuarter(state.regSecondsRemaining)}`:`Play Overtime ${state.ot?.period||1}`;}
...
function renderScore(){
  const hasOt=state.overtime.away.length>0||state.overtime.home.length>0;
  $('scoreboard').classList.toggle('has-ot',hasOt);
  const headers='<div></div><b>Q1</b><b>Q2</b><b>Q3</b><b>Q4</b>'+(hasOt?'<b>OT</b>':'')+'<b>F</b>';
  const teamRow=(side,prefix)=>{
    const cells=state.quarters[side].map((score,i)=>{
      const begun=state.final||Boolean(state.periodsStarted?.[i]);
      return `<span id="${prefix}${i+1}" data-side="${side}" data-quarter="${i+1}">${begun?score:'-'}</span>`;
    }).join('');
    const ot=hasOt?`<span>${state.overtime[side].reduce((a,b)=>a+b,0)}</span>`:'';
    return `<strong class="team" id="${prefix}n">${esc(state.teams[side].code)}</strong>${cells}${ot}<strong id="${prefix}f">${state.score[side]}</strong>`;
  };
  $('scoreboard').innerHTML=headers+teamRow('away','a')+teamRow('home','h');
}

function renderStatus(){
  if(state.final){
    $('status').innerHTML=`<b>FINAL - ${esc(state.teams[state.winner].code)} ${state.score[state.winner]}-${state.score[state.winner==='away'?'home':'away']}</b><div class="small">${state.events.length} completed possessions - ${state.changes.length} live-control changes</div>`;
    return;
  }
  let text;
  if(state.pausedAfterQuarter&&state.regSecondsRemaining>0){
    text=state.pausedAfterQuarter===2?'Halftime - End of Q2 (0:00)':`End of Q${state.pausedAfterQuarter} (0:00)`;
  }else if(state.regSecondsRemaining>0){
    const c=quarterClock(state.regSecondsRemaining);
    text=`Q${c.quarter} ${clock(c.seconds)} - ${state.teams[state.currentSide].code} ball - ${fieldLabel(state.yardsToGoal)}`;
  }else text=`Regulation complete - OT${state.ot?.period||1} - ${state.teams[state.currentSide].code} possession`;
  const d=state.pendingDrive;
  const pending=d?`Drive #${state.nextPossession} continues next quarter; no result or points committed yet.`:`Next possession #${state.nextPossession}`;
  const change=state.changes.at(-1);
  const queued=change&&change.effectiveFromPossession>=state.nextPossession?` Controls apply from possession #${change.effectiveFromPossession}.`:'';
  $('status').innerHTML=`<b>${esc(text)}</b><div class="small">${esc(pending+queued)}</div>`;
}

function renderTransparency(){let html=state.pendingDrive?'<p class="small">Current drive ratings remain locked. New controls apply to the next possession.</p>':'';if(!state.final&&state.regSecondsRemaining>0){const q=regulationQuarter(state.regSecondsRemaining),m=state.pendingDrive?.probabilityModel||driveProbabilities(state,state.currentSide,state.yardsToGoal,q),side=state.currentSide,opp=side==='away'?'home':'away';html+=`<div class="metrics"><div class="metric"><span class="small">${state.teams[side].code} base TEAM / OFF / DEF</span><b>${fmt3(state.teams[side])}</b><span class="small">Effective ${fmt3(m.effective[side])}</span></div><div class="metric"><span class="small">${state.teams[opp].code} base TEAM / OFF / DEF</span><b>${fmt3(state.teams[opp])}</b><span class="small">Effective ${fmt3(m.effective[opp])}</span></div><div class="metric"><span class="small">Drive strength</span><b>${m.strength.total.toFixed(2)}</b><span class="small">TEAM gap ${m.strength.teamGap.toFixed(2)} · unit residual ${m.strength.unitResidual.toFixed(2)} · site ${m.strength.siteStrength.toFixed(2)}</span></div><div class="metric"><span class="small">Scoring opportunity</span><b>${(m.pScore*100).toFixed(1)}%</b><span class="small">Field logit ${m.fieldLogit.toFixed(3)} · weather ${m.weather.total.toFixed(3)}</span></div><div class="metric"><span class="small">Manual / Comeback / Applied</span><b>${m.effective.manual.toFixed(2)} / ${m.effective.comeback.signed.toFixed(2)} / ${m.effective.combined.toFixed(2)}</b><span class="small">Clipped ${m.effective.clipped.toFixed(2)}</span></div><div class="metric"><span class="small">Expected final from forecast</span><b>${lastForecast?`${state.teams.away.code} ${lastForecast.meanFinalAway.toFixed(1)} · ${state.teams.home.code} ${lastForecast.meanFinalHome.toFixed(1)}`:'Pending'}</b></div></div><details><summary>Next-drive probability vector</summary><div class="json">${esc(JSON.stringify(m.probabilities,null,2))}</div></details>`;}else if(state.events.length){html=`<div class="small">Final deterministic gameplay hash</div><div class="json">${esc(state._archivedGameplayHash||gameplayStateHash(state))}</div>`;}$('transparency').innerHTML=html;}
...
async function exactReplay(i){try{const a=getHistory()[i];
  const replay=a.identity?.engine==='PC-MOBILE-v2.0.4.2-R1-PROTOTYPE'
    ?(await import('./engine_r1_legacy.mjs?v=bootfix1')).replayAudit:replayAudit;
  const s=replay(rows,a);s._archivedGameplayHash=a.gameplayHash;state=s;state._saved=true;for(const k of ['away','home'])$(k).value=a.initial[k];$('seed').value=a.initial.seed;$('neutral').checked=a.initial.neutral;$('temperature').value=a.initial.temperature;$('precipitation').value=a.initial.precipitation;$('evenTeams').checked=a.initial.evenTeams;$('manualLevel').value=a.initial.manualLevel;$('prototype').value=a.initial.prototype;$('comebackEnabled').checked=a.initial.comebackEnabled;locked(true);lastForecast=null;$('gameCard').classList.remove('hidden');render();document.querySelector('[data-tab="game"]').click();}catch(e){showError(e);}}
function rerunSeed(i){const a=getHistory()[i];newGame();$('away').value=a.initial.away;$('home').value=a.initial.home;$('seed').value=a.initial.seed;document.querySelector('[data-tab="game"]').click();updateAdvReadout();}
